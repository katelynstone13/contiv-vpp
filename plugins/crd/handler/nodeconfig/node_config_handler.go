...
// objects to node config protobuf objects.
...
func (h *Handler) Init() error {
	...
	h.kpc = func(obj interface{}) (interface{}, string, bool) {
		...
		return h.nodeConfigToProto(nodeConfig), model.Key(nodeConfig.Name), true
	}
...
func (h *Handler) ObjectDeleted(obj interface{}) {
	...
	nodeConfigProto := h.nodeConfigToProto(nodeConfig)
	_, err := h.Publish.Delete(model.Key(nodeConfig.GetName()))
	if err != nil {
		h.Log.WithField("rwErr", err).
			Warnf("node config failed to delete item from data store: %v", nodeConfigProto)
	}
}
...
func (h *Handler) ObjectUpdated(oldObj, newObj interface{}) {
	...
	if !reflect.DeepEqual(oldObj, newObj) {
		...
		h.Log.Debugf("node config updating item in data store, %v", newObj)
